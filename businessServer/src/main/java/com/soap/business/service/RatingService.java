...
import com.soap.business.model.domain.Rating;
import com.soap.business.model.domain.User;
import com.soap.business.model.request.MovieRatingRequest;
...
public class RatingService {
    ...
    public boolean newRating(Rating rating) {
        jdbcTemplate.update("insert into " + Constants.DB_RATING() + "(uid,mid,score,timestamp) values(?,?,?,?)",
                new Object[]{rating.getUid(), rating.getMid(), rating.getScore(), rating.getTimestamp() / 1000});
        return true;
    }
    ...
    public int[] getMyRatingStat(User user) {
        ...
        ratings = jdbcTemplate.query("select * from " + Constants.DB_RATING() + " where uid=" + user.getUid(), new RowMapper<Rating>() {
            @Override
